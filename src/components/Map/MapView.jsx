...
function MapInner() {
  ...
  return (
    <>
      ...
      {/* City markers and custom drawn features */}
      <CityLayer />
      <FeatureLayer />
      <DrawingTools />
      <DiplomaticLines />
...
export default function MapView() {
  const {
    ...
    setFeatures, setCities,
    setCurrentTurn,
    layers,
    overlayOpacity,
  } = useMapStore();
  const [imgSize, setImgSize] = useState({ w: DEFAULT_W, h: DEFAULT_H });
  ...
  useEffect(() => {
    ...
    return () => { unsubSettings(); unsubFeatures(); unsubCities(); };
  }, []);
  ...
  const placeholderSvg = `<svg xmlns="http://www.w3.org/2000/svg" width="${imgSize.w}" height="${imgSize.h}"><rect width="100%" height="100%" fill="#374151"/><text x="50%" y="50%" fill="#9CA3AF" font-size="32" text-anchor="middle" dominant-baseline="middle" font-family="sans-serif">地図画像が設定されていません</text></svg>`;
  ...
  return (
    <div className="absolute inset-0">
      ...
      >
        ...
        {layers.border        && <ImageOverlay url={traceUrl('border.png')}         bounds={bounds} opacity={overlayOpacity} />}
        ...
        <MapBoundsFitter h={imgSize.h} w={imgSize.w} />
        <MapInner />
      </MapContainer>
    </div>
  );
